...
class OverviewRepository
{
    public function run()
    {
        ...
        $model = new Course();
        $hasMoreThanOne = $model->exec("select count(p1.id) as total, YEAR(nextshow) as year, MONTH(nextshow) as month, DAY(nextshow) as day, p1.* from playedcard p1 where user_id = ? and id <> (select MAX(p2.id) from playedcard p2 where p1.card_id = p2.card_id) group by card_id, YEAR(nextshow), MONTH(nextshow), DAY(nextshow)", [$user['id']]);
        ...
        return ['played' => $resultPlayed, 'to_play' => $toPlay];
    }
}
