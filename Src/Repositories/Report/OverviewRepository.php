...
class OverviewRepository
{
    public function run()
    {
        ...
        $toPlay = $model->exec("select count(p1.id)as total, YEAR(nextshow) as year, MONTH(nextshow) as month, DAY(nextshow) as day, p1.created_at, p1.nextshow from playedcard p1 where user_id = ? and id = (select MAX(p2.id) from playedcard p2 where p1.card_id = p2.card_id limit 1) group by YEAR(nextshow), MONTH(nextshow), DAY(nextshow)", [$user['id']]);
